    ...
    <div className="h-full flex flex-col bg-slate-50 dark:bg-slate-950">
      ...
      >
        <div className="flex items-center justify-between">
          <div>
            <h1 className="text-3xl font-bold text-slate-900 dark:text-white mb-2">Execution Settings</h1>
            <p className="text-slate-600 dark:text-slate-400">Configure test execution preferences</p>
          </div>
          <motion.button
        ...
        <div className="max-w-4xl mx-auto space-y-6">
          ...
          >
            <div className="flex items-center gap-3 mb-6">
              ...
              <div>
                <h3 className="text-lg font-semibold text-slate-900 dark:text-white">Browser</h3>
                <p className="text-sm text-slate-600 dark:text-slate-400">Select the browser for test execution</p>
              </div>
            </div>
            <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
              {browsers.map((browser) => (
                <motion.button
                  ...
                  whileTap={{ scale: 0.98 }}
                  onClick={() => {
                    updateSettings({ browser: browser.value });
                    addToast({
                      type: 'info',
                      message: `Browser changed to ${browser.label}`,
                    });
                  }}
                  className={cn(
                    ...
                    settings.browser === browser.value
                      ...
                      : 'bg-slate-50 dark:bg-slate-800/50 border-slate-300 dark:border-slate-700 hover:border-slate-400 dark:hover:border-slate-600'
                  )}
                >
                  ...
                  <p
                    className={cn(
                      'text-sm font-medium',
                      settings.browser === browser.value ? 'text-violet-600 dark:text-violet-400' : 'text-slate-700 dark:text-slate-300'
                    )}
                  >
                    {browser.label}
                  </p>
          ...
          {settings.autoRetry && (
            ...
            >
              <div className="flex items-start justify-between gap-4">
                <div className="flex-1">
                  <h3 className="text-lg font-semibold text-slate-900 dark:text-white mb-1">Max Retries</h3>
                ...
                <div className="flex items-center gap-3">
                  <button
                    ...
                    className="w-8 h-8 rounded-lg bg-slate-200 dark:bg-slate-800 hover:bg-slate-300 dark:hover:bg-slate-700 text-slate-900 dark:text-white transition-colors flex items-center justify-center"
                  >
                    -
                  </button>
          ...
          >
            <motion.button
              ...
              whileTap={{ scale: 0.95 }}
              onClick={handleReset}
              className="flex items-center gap-2 px-6 py-3 bg-slate-200 dark:bg-slate-800 hover:bg-slate-300 dark:hover:bg-slate-700 text-slate-700 dark:text-slate-300 hover:text-slate-900 dark:hover:text-white rounded-lg font-medium transition-all"
            >
